 ...
 */
static
bool avl_insert(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t));
...
// Check if a node is part of an AVL Tree
static bool is_avl_node(avl_node_t *node)
{
    // Base case 1: Tree is empty
    if (node == NULL){
        return true;
    }
    // Base case 2: Check height of left and right subtree
    else if ( abs((HEIGHT(node->left) - HEIGHT(node->right))) >= 2 ){
        return false;
    }
    // Recurse with left and right subtree
    else{
        return is_avl_node(node->left) && is_avl_node(node->right);
    }
}
...
// Insert element into a BST without performing rotations
static bool bst_insert(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t))
{
    bool inserted;
    if (! *root) {
        inserted = (*root = avl_node_create(elem));
    } else if ((*cmp)(elem, (*root)->elem) <= 0) {
        if ((inserted = bst_insert(&(*root)->left, elem, cmp))) {
            avl_update_height(*root);
        }
    } else if ((*cmp)(elem, (*root)->elem) > 0) {
        if ((inserted = bst_insert(&(*root)->right, elem, cmp))) {
            avl_update_height(*root);
        }
    }
    return inserted;
...
{
    if (bst_insert(&(bag->root), elem, bag->cmp)){
        bag->size++;
        return true;
    } else{
        return false;
...
/* REMOVE FUNCTION
- if root == NULL, nothing to remove; return false
- check if element to remove is less than, greater than, or equal to root
- if less than, apply the function recursively to left side
- if greater than, apply function recursively to right side
- if equal to, check condition of left and right subtrees:
    - if both exist, remove max/min from left/right subtree, whichever one is taller
    - if only one exists, remove the root */
...
bool my_avl_remove(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t))
{
    ...
    if (! *root){
        ...
        if (removed = my_avl_remove(&(*root)->left, elem, cmp)){
        ...
        if (removed = my_avl_remove(&(*root)->right, elem, cmp)){
            // rebalance and update height
            if ( abs((HEIGHT((*root)->left) - HEIGHT((*root)->right))) >= 2 ){
                avl_rebalance_to_the_right(root);
            }
        ...
        if ((*root)->left && (*root)->right){
            if (HEIGHT((*root)->left) > HEIGHT((*root)->right)){
                ...
                (*root)->elem = avl_remove_min(&(*root)->right);
            } // don't need to rebalance or update height - avl_remove_max/min functions do it
        }
        else{ //remove root, replace with whichever branch exists
            avl_node_t *old = *root;
            *root = (*root)->left ? (*root)->left : (*root)->right;
            free(old);
        }
...
bool my_bag_remove(bag_t *bag, bag_elem_t elem)
{
    if (my_avl_remove(&bag->root, elem, bag->cmp)) {
        bag->size--;
        return true;
    } else {
        return false;
